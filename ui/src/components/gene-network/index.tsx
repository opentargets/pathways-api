...
const PathwaysGeneNetwork: React.FC<PathwaysGeneNetworkProps> = ({ pathways }) => {
  ...
  const getLayoutedElements = useCallback((nodes: any[], edges: any[], direction = 'TB') => {
    ...
    dagreGraph.setGraph({ rankdir: direction, ranksep: settings.nodeSpacing, nodesep: 80 });
        ...
        <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          {networkData.renderTime > 0 && (
            <Chip 
              ...
              color={networkData.renderTime < 16 ? 'success' : networkData.renderTime < 33 ? 'warning' : 'error'}
              size="small" 
            />
          )}
          ...
          {edges.length > 0 && (
            <Box sx={{ display: 'flex', gap: 0.5, alignItems: 'center', ml: 2 }}>
              ...
              {Array.from(networkData.colorMapping.values()).slice(0, 8).map((color, index) => (
                <Box
                  key={index}
                  sx={{
                    width: 12,
